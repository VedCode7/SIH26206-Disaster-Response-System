...
(function () {
    ...
    let view = { scale: 1.24, x: 0, y: 0, dragging: false, lastX: 0, lastY: 0 };
    let bounds = null;
    ...
    const CSS = `
        ...
        .road-canvas-wrap{position:absolute;inset:0;background:#040a10;overflow:hidden;cursor:grab}.road-canvas-wrap.dragging{cursor:grabbing}
        ...
        .road-health{margin-top:12px;padding:12px;border:1px solid #19303c;background:#0b161f;border-radius:8px}.road-health-head{display:flex;justify-content:space-between;align-items:center}.road-health-head span{font-size:8px;color:#708899;letter-spacing:.8px}.road-health-head strong{font-size:11px;color:#19e6a0}.road-health-track{height:5px;margin-top:9px;background:#162631;border-radius:5px;overflow:hidden}.road-health-fill{height:100%;background:linear-gradient(90deg,#ff4055,#ffad28,#15e39a);border-radius:5px;transition:width .45s ease}
    ...
    function drawGrid(width, height) {
        ...
        ctx.lineWidth = 2;
        for (let x = ((scanPhase * 0.45) % 120) - 120; x < width; x += 120) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x + 170, height); ctx.stroke(); }
        ctx.restore();
    }

    function drawRoad(road, width, height, emphasis) {
        ...
        const selected = selectedRoad && selectedRoad.id === road.id;
        const hovered = hoveredRoad && hoveredRoad.id === road.id;
        const highlighted = emphasis || selected || hovered;
        ...
        if (highlighted) {
            ...
            ctx.globalAlpha = selected ? .72 : .25;
        ...
        ctx.save();
        ctx.strokeStyle = color;
        ctx.globalAlpha = selected ? 1 : hovered ? .98 : status === "open" ? .72 : .9;
        const baseWidth = status === "open" ? 1.05 : 1.25;
        ctx.lineWidth = selected ? 2.5 : hovered ? 2 : Math.max(.8, baseWidth / Math.max(view.scale * .88, .8));
        if (status === "blocked") ctx.setLineDash([4, 5]);
        ctx.stroke();
        ctx.restore();
    }

    function drawNetworkHalo(width, height) {
        if (!bounds) return;
        const center = project((bounds.minLon + bounds.maxLon) / 2, (bounds.minLat + bounds.maxLat) / 2, width, height);
        ...
        gradient.addColorStop(0, "rgba(0,188,255,.045)");
        gradient.addColorStop(.55, "rgba(0,188,255,.018)");
        gradient.addColorStop(1, "rgba(0,188,255,0)");
        ctx.save(); ctx.fillStyle = gradient; ctx.fillRect(0, 0, width, height); ctx.restore();
    }

    function drawScan(width, height) {
        if (!canvas) return;
        const x = (scanPhase % (width + 220)) - 110;
        ctx.save();
        const grad = ctx.createLinearGradient(x - 65, 0, x + 65, 0);
        grad.addColorStop(0, "rgba(17,190,244,0)");
        grad.addColorStop(.5, "rgba(17,190,244,.075)");
        grad.addColorStop(1, "rgba(17,190,244,0)");
        ctx.fillStyle = grad; ctx.fillRect(x - 65, 0, 130, height);
        ctx.restore();
    }
    ...
    function drawMap() {
        ...
        // Base network pass: restrained glow keeps the entire topology legible.
    ...
    function pickRoad(clientX, clientY) {
        ...
        let best = null, bestDistance = 12 / view.scale;
    ...
    function fitMap() { view.scale = 1.24; view.x = 0; view.y = 0; drawMap(); }
    function zoom(delta) { view.scale = Math.max(.55, Math.min(4.5, view.scale + delta)); drawMap(); }
    ...
    function selectedRoadHtml(road) {
        ...
        const status = classifyRoad(road);
        return `<div class="road-detail-main">
            <div class="road-detail-top"><div><div class="road-detail-id">${escapeHTML(road.id)}</div><div class="road-detail-route">${escapeHTML(road.from_zone_id)} → ${escapeHTML(road.to_zone_id)}</div></div><span class="road-detail-status road-status-${status}">${roadStatusLabel(status)}</span></div>
    ...
    function buildView(roads) {
        currentRoads = roads; bounds = computeBounds(roads);
        const counts = statusCounts(roads), avg = averageAccessibility(roads);
        ...
        const map = `<div class="road-command-page">
            <div class="road-command-head"><div><p class="eyebrow">DISASTER-AWARE ROUTING</p><h2>Road Network</h2><p class="subtitle">Live road geometry, accessibility and network condition across the persisted Chennai road graph.</p></div>
            <div class="road-command-live"><i class="road-live-dot"></i><div><strong>NETWORK TELEMETRY</strong><span>LIVE · ${roads.length.toLocaleString()} LINKS</span></div></div></div>
            <div class="road-command-shell">
                <section class="road-visual-card"><div class="road-visual-toolbar"><div class="road-visual-title"><b>LIVE GEOSPATIAL FEED</b> · CHENNAI ROAD GRAPH</div><div class="road-visual-tools"><button class="road-map-btn" data-road-action="zoom-in" title="Zoom in">+</button><button class="road-map-btn" data-road-action="zoom-out" title="Zoom out">−</button><button class="road-map-btn" data-road-action="reset" title="Reset view">⌖</button></div></div>
                <div class="road-canvas-wrap" id="road-canvas-wrap"><canvas class="road-canvas" id="road-network-canvas" aria-label="Live Chennai road network visualisation"></canvas><div class="road-map-scan" id="road-map-scan"></div></div>
                <div class="road-map-hud"><strong>GEOSPATIAL TELEMETRY</strong><span>Drag to pan · wheel to zoom · click a segment to inspect</span></div><div class="road-map-legend"><span><i class="road-key-open"></i>Open</span><span><i class="road-key-limited"></i>Limited</span><span><i class="road-key-degraded"></i>Degraded</span><span><i class="road-key-blocked"></i>Blocked</span></div></section>
                <aside class="road-telemetry-card"><div class="road-card-heading"><div><span class="road-card-kicker">NETWORK CONDITION</span><h3>Operational Telemetry</h3></div><span class="road-filter-count" id="road-filter-count">${filtered.toLocaleString()} visible</span></div>
                    <div class="road-summary-grid"><div class="road-summary-tile open"><small>OPEN LINKS</small><strong>${counts.open.toLocaleString()}</strong><em>≥ 75% access</em></div><div class="road-summary-tile warn"><small>LIMITED</small><strong>${counts.limited.toLocaleString()}</strong><em>50–74% access</em></div><div class="road-summary-tile danger"><small>DEGRADED / BLOCKED</small><strong>${(counts.degraded + counts.blocked).toLocaleString()}</strong><em>requires attention</em></div><div class="road-summary-tile info"><small>MEAN ACCESS</small><strong>${avg.toFixed(1)}%</strong><em>network-wide</em></div></div>
                    ...
                    <div class="road-filter-bar">${["all","open","limited","degraded","blocked"].map((filter) => `<button class="road-filter ${activeFilter === filter ? "active" : ""}" data-road-filter="${filter}">${filter === "all" ? "ALL" : filter.toUpperCase()}</button>`).join("")}</div>
                    <div class="road-detail-card" id="road-detail-card">${selectedRoadHtml(selectedRoad)}</div>
                </aside>
            </div>
            <div class="road-network-strip"><div class="road-strip-card accent"><small>ACTIVE ROAD LINKS</small><strong>${roads.length.toLocaleString()}</strong><span>Persisted geospatial records</span></div><div class="road-strip-card"><small>TRAVERSABLE</small><strong>${operational.toLocaleString()}</strong><span>Not blocked</span></div><div class="road-strip-card"><small>RESTRICTED</small><strong>${restricted.toLocaleString()}</strong><span>Accessibility below 75%</span></div><div class="road-strip-card"><small>BLOCKED</small><strong>${counts.blocked.toLocaleString()}</strong><span>Excluded from routing</span></div></div>
        ...
        if (main) main.innerHTML = `<section class="page-content road-page-content">${map}</section><footer>SIH26206 • Disaster Response Decision Support System</footer>`;
        setActiveNavigationSafe("roads"); bindView(); fitMap(); startMap();
    }
    ...
    function bindView() {
        canvas = document.getElementById("road-network-canvas"); viewport = document.getElementById("road-canvas-wrap"); ctx = canvas ? canvas.getContext("2d") : null;
        if (!canvas || !viewport || !ctx) return;

        document.querySelectorAll("[data-road-action]").forEach((button) => button.addEventListener("click", () => { const action = button.dataset.roadAction; if (action === "zoom-in") zoom(.25); else if (action === "zoom-out") zoom(-.25); else fitMap(); }));
        document.querySelectorAll("[data-road-filter]").forEach((button) => button.addEventListener("click", () => {
            activeFilter = button.dataset.roadFilter; selectedRoad = null;
            document.querySelectorAll("[data-road-filter]").forEach((b) => b.classList.toggle("active", b === button));
            const countEl = document.getElementById("road-filter-count"); if (countEl) countEl.textContent = `${currentRoads.filter(roadMatchesFilter).length.toLocaleString()} visible`;
            const detail = document.getElementById("road-detail-card"); if (detail) detail.innerHTML = selectedRoadHtml(null); drawMap();
        }));

        canvas.addEventListener("wheel", (event) => { event.preventDefault(); zoom(event.deltaY < 0 ? .16 : -.16); }, { passive: false });
        canvas.addEventListener("pointerdown", (event) => { view.dragging = true; view.lastX = event.clientX; view.lastY = event.clientY; viewport.classList.add("dragging"); canvas.setPointerCapture(event.pointerId); });
        canvas.addEventListener("pointermove", (event) => {
            if (view.dragging) { view.x += event.clientX - view.lastX; view.y += event.clientY - view.lastY; view.lastX = event.clientX; view.lastY = event.clientY; drawMap(); return; }
            const hit = pickRoad(event.clientX, event.clientY); if ((hit?.id || null) !== (hoveredRoad?.id || null)) { hoveredRoad = hit; drawMap(); }
        });
        canvas.addEventListener("pointerup", (event) => { if (view.dragging) { view.dragging = false; viewport.classList.remove("dragging"); try { canvas.releasePointerCapture(event.pointerId); } catch (_) {} } });
        canvas.addEventListener("pointerleave", () => { if (!view.dragging) { hoveredRoad = null; drawMap(); } });
        canvas.addEventListener("click", (event) => { if (view.dragging) return; const hit = pickRoad(event.clientX, event.clientY); selectedRoad = hit; const detail = document.getElementById("road-detail-card"); if (detail) detail.innerHTML = selectedRoadHtml(selectedRoad); drawMap(); });

        resizeObserver = new ResizeObserver(resizeCanvas); resizeObserver.observe(viewport);
    }

    function startMap() { if (animationFrame) cancelAnimationFrame(animationFrame); scanPhase = 0; animationFrame = requestAnimationFrame(animate); }
    ...
    async function showRoadNetworkVisual() {
        ...
        try {
            ...
            const main = document.querySelector(".main-content");
            if (main) main.innerHTML = `<section class="page-content"><div class="error-card"><strong>Road network unavailable</strong><span>${escapeHTML(error.message)}</span></div></section><footer>SIH26206 • Disaster Response Decision Support System</footer>`;
        }
    }

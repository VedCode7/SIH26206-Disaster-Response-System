...
def calculate_route(
    ...
    """
    Calculate the fastest currently traversable route between two zones
    ...
    while priority_queue:
        current_distance, current_zone = heapq.heappop(
            priority_queue
        )

        if current_distance > distances.get(current_zone, float("inf")):
            continue
        ...
        for road in graph.neighbors(current_zone):
            ...
            new_distance = current_distance + road.travel_time_min

            if new_distance < distances.get(
                next_zone,
                float("inf"),
            ):
                distances[next_zone] = new_distance
                previous_zone[next_zone] = current_zone
                previous_road[next_zone] = road

                heapq.heappush(
                    priority_queue,
                    (new_distance, next_zone),
                )
    ...
    return RouteResult(
        ...
        total_travel_time_min=distances[destination_zone_id],
    )
...
def _facility_context(
    ...
    graphs_by_stage: list[RoutingGraph],
) -> None:
    """Attach route-aware real facility recommendations to each response stage."""
    for stage, graph in zip(response["stages"], graphs_by_stage):
        ...
        for priority_zone in stage["response"]["priority_zones"]:
            ...
            relevant = filter_relevant_facilities(
                facilities,
                # Risk level is serialized as a string in the simulation output.
                # The response object already contains the canonical value.
                next(
        ...
        stage["response"]["mapped_facilities"] = [
            ...
            if facility.current_zone_id in priority_zone_ids
        ]
...
def run_chennai_2015_response_simulation(
    *,
    ...
    # Attach facility recommendations using the same stage-specific route
    # state used for resource deployment.
